...
// ---------- orchestrator ----------
export const startRender = onCall(async (request: any) => {
...
export const connectYoutube = onCall(async (request: any) => {
  const uid = request.auth?.uid;
  if (!uid) throw new HttpsError('unauthenticated', 'Log in first.');
...
export const youtubeOAuthCallback = onRequest(async (req: any, res: any) => {
...
export const connectSocial = onCall(async (request: any) => {
  const uid = request.auth?.uid;
...
export const publishVideo = onCall(async (request: any) => {
  const uid = request.auth?.uid;
